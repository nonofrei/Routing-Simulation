...
def receive_packet(connection, max_buffer_size):
    ...
    write_to_file("../output/received_by_router_3.txt", received_packet)
    # Split the packet by the delimiter.
    packet = received_packet.split(',')
    # Return the list representation of the packet.
    if len (packet) <= 1:
        return None
    return packet
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    while True:
        ...
        print("OUT:", packet[2])
        write_to_file("../output/out_router_3.txt", packet[2])
