...
def receive_packet(connection, max_buffer_size):
    ...
    write_to_file("../output/received_by_router_4.txt", received_packet)
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    soc_8005 = create_socket("localhost", 8005)
    soc_8006 = create_socket("localhost", 8006)
    ...
    while True:
        ...
        # Send the new packet to the appropriate port (and append it to sent_by_router_4.txt),
        ...
        if new_ttl == "0" and port != "127.0.0.1":
            ...
            write_to_file("../output/sent_by_router_4.txt", new_packet, port)
            soc_8005.send(new_packet.encode())
        elif port == "8006":
            print("sending packet", new_packet, "to Router 6")
            write_to_file("../output/sent_by_router_4.txt", new_packet, port)
            soc_8006.send(new_packet.encode())
        else:
            print("OUT:", payload)
